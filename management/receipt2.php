...
//Receipt 2 with QR Code Uploaded on 25122020 
...
//echo "<pre>"; print_r($customer_details); die;
    ...
    $receipt_number_detail = getReceiptNumberDetail($conn,$transaction_id);
    // print_r($receipt_number_detail);die;
    $block_number_id = $receipt_number_detail['block_number_id'];
    // $block_number_id = blockNumberName($conn,$block_number_ids);
    // echo $block_number_id;
    $block_id = $receipt_number_detail['block_id'];

    $customer_id = $receipt_number_detail['customer_id'];
    $customer_name = customerName($conn,$customer_id);
    $customerDetail = customerDetail($conn,$customer_id);
    $customer_mobile = isset($customerDetail['mobile'])?$customerDetail['mobile']:'';
    $project_name = blockProjectName($conn,$receipt_number_detail['block_id']);
    
   // include php barcode 128 class
  $amount = number_format($transaction_details['amount'],2);
  $paid_date = date("d.m.Y",strtotime($transaction_details['paid_date']));
  $date = date("d.m.Y");
  $s = $receipt_number_detail['id']."-".$customer_id."-".$block_id."-".$receipt_number_detail['block_number_id']."-".$receipt_number_detail['transaction_id']."-".receiptNumber($conn,$receipt_number_detail['transaction_id'])."-".customerID($customer_details['id'])."-".$customer_name."-".$customer_mobile."-".$project_name."-".$amount."-".$paid_date."-".$date;
  $PNG_TEMP_DIR = dirname(__FILE__).DIRECTORY_SEPARATOR.'temp'.DIRECTORY_SEPARATOR;
    ...
    $matrixPointSize = 2;
        $filename = $PNG_TEMP_DIR.'KCCODE'.md5($s.'|'.$errorCorrectionLevel.'|'.$matrixPointSize).'.png';
        QRcode::png($s, $filename, $errorCorrectionLevel, $matrixPointSize, 2);    
   $qrcode = '<img src="'.$PNG_WEB_DIR.basename($filename).'" /><hr/>';  
    ...
    <table width="92%" border="0" style="margin-left:auto; margin-right:auto;">
      <tbody>
        ...
        <tr>
          ...
          <td>&nbsp;</td>
          <td  >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
          <td colspan="3" nowrap>Authorized Signature &nbsp;&nbsp;&nbsp; <?php echo $qrcode?></td>
        </tr>
      </tbody>
    </table>
