...
if(!(userCan($conn,$_SESSION['login_id'],$privilegeName = 'manage_neft_report'))){ 
 	header("location:/wcc_real_estate/index.php");
 	exit();
}

$url = 'neft_report.php?search=Search';
...
if(isset($_POST['cancelTransaction'])){ 
	...
	if($error){
		...
		mysqli_commit($conn);
		$_SESSION['success'] = 'Neft has been cancelled Successfully.';
	}
	mysqli_close($conn);
}
...
if(isset($_POST['markClear'])){
	...
	// echo "<pre>";print_r($clear_date);die;
	...
	if($error){
		...
		mysqli_commit($conn);
		$_SESSION['success'] = 'Neft has been cleared Successfully.';
	}
	mysqli_close($conn);
	}
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Neft Payment Report</li>
          </ol>
        </section>
        ...
        <section class="content">
			<div class="box">
                <div class="box-header">
					...
					<div class="row">
						<div class="col-sm-10">
							<h3 class="box-title">Neft Payment Report</h3>
						</div>
						<div class="col-sm-1">
								<a href="<?php echo str_replace("neft_report","neft_report_excel",$url) ?>" class="btn btn-sm btn-success pull-right"><i class="fa fa-file-excel-o"></i> Excel Export</a>
						</div>
					...
					<form class="" action="neft_report.php" name="search_frm" id="search_frm" method="get">

						<div class="form-group col-sm-3">
							<lable for ="search_cheque_dd_number" ><b>Neft No</b></lable>
							...
							if(mysqli_num_rows($customers) > 0){
								...
								while($customer = mysqli_fetch_assoc($customers)){
									...
									<tr>
										...
										<td>
											<strong>Name : </strong><?php echo ($customer['customer_name_title'].' ' .$customer['customer_name']).'<br>'.' ('.customerID($customer['customer_id']).')';
											?><br>
										...
										<td>
											<?php if(userCan($conn,$_SESSION['login_id'],$privilegeName = 'neft_cancel_neft_report')){  ?>
											<a onclick="cancelTransaction('<?php echo $customer['id']; ?>','<?php echo customerID($customer['customer_id']); ?>','<?php echo $customer['block_id']; ?>','<?php echo $customer['block_number_id']; ?>');" href="javascript:void(0)" class="btn btn-xs btn-danger" data-toggle="tooltip" title="Cancel neft"><i class="fa fa-remove"></i></a>
										<?php } if(userCan($conn,$_SESSION['login_id'],$privilegeName = 'neft_clear_neft_report')){ ?>
		...
		<div class="modal-content">
			<form enctype="multipart/form-data" action="<?php echo $page_url; ?>" name="add_late_payment_frm" id="add_late_payment_frm" method="post" class="form-horizontal dropzone">
			  <div class="modal-header">
				...
				<h4 class="modal-title">Cancel Neft</h4>
			  </div>
			  <div class="modal-body">
				<div class="box box-info">
					<div class="box-header with-border">
						<div class="col-md-12">
							<h3 class="box-title">Cancel Neft Panel</h3>
						</div>
					</div><!-- /.box-header -->
					<!-- form start -->
					<div class="box-body">
	  ...
	  <div class="modal-dialog">
		<div class="modal-content">
			<form enctype="multipart/form-data" action="<?php echo $page_url; ?>" name="add_late_payment_frm" id="add_late_payment_frm" method="post" class="form-horizontal dropzone">
			  <div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title">Neft Mark Clear</h4>
			  </div>
			  <div class="modal-body">
				<div class="box box-info">
					<div class="box-header with-border">
					...
					<div class="box-body">
						<div class="form-group">
						  <label for="bank_name" class="col-sm-3 control-label">Neft Clear Date<sup class="text-danger text-lg">*</sup></label>
						  <div class="col-sm-8">
