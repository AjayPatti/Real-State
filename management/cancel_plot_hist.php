...
$url = 'cancel_plot_hist.php?search=Search';
$limit = 100;
...
if (isset($_POST['addTransaction']) && $_POST['addTransaction'] == "Save Changes") {
    ...
    $refund_date =     filter_post($conn, date("Y-m-d", strtotime($_POST['refund_date'])));
    ...
    $total_deposit = mysqli_fetch_assoc(mysqli_query($conn, "select sum(amount) as Deposited from kc_customer_transactions_hist where customer_id = '" . $block_number['customer_id'] . "' and block_id = '" . $block_number['block_id'] . "' and block_number_id = '" . $block_number['block_number_id'] . "' and cr_dr = 'cr' and action_type = 'Cancel Booking' and  batch='".$block_number['batch']."' "));
     // addedon <= '" . $block_number['addedon'] . "' and deleted >= '" . $block_number['deleted'] .
    ...
    // echo "<pre>";print_r($refunded_amount);die;
    if ($refund_amount == '') {
    ...
    } else if ($refund_date == '' || $refund_date == '1970-01-01') {
        $_SESSION['error'] = 'Refund Date was required!';
        ...
        mysqli_query($conn, "insert into kc_refund_amount set customer_blocks_hist_id = '$refund_id', customer_id = '$refund_customer_id', block_id = '$refund_block_id', block_number_id = '$refund_block_number_id', amount = '$refund_amount',refund_date ='$refund_date' , remark = '$refund_remarks', status = '1', added_by = '" . $_SESSION['login_id'] . "', addedon = '" . date('Y-m-d H:i:s') . "' ");
...
<body class="skin-blue sidebar-mini">
                            ...
                            <table class="table table-striped table-hover table-bordered">
                                ...
                                if ($search && mysqli_num_rows($customers) > 0) {
                                    ...
                                    while ($customer = mysqli_fetch_assoc($customers)) {
                                        ...
                                        //Above query commented on 24032023 because two duplicate(customer_id, block_id, block_number_id) were calculated in same transactions

                                        $total_refund = mysqli_fetch_assoc(mysqli_query($conn, "select sum(amount) as total_refunded from kc_refund_amount where customer_blocks_hist_id = '" . $customer['customer_block_id'] . "' and deleted is null"));  
                                        ...
                                        // $associate_name = $customer['associate_code'].'-'.$customer['associate_name'];
                                        $associate_name = associateName($conn,$customer['associate']);
                                        // echo "<pre>"; print_r($associate_name);die;
                                        ...
                                        <tr>
                                            ...
                                            <td width="8%">
                                                <?php if (userCan($conn, $_SESSION['login_id'], $privilegeName = 'view_transaction_cancel_plot_hist')) {  ?>
                                                    ...
                                                    <?php }
                                                if (userCan($conn, $_SESSION['login_id'], $privilegeName = 'add_refund_cancel_plot_hist')) {
                                                    if ($pending_amount > 0) { ?>
                                                        <button class="btn btn-xs btn-primary" type="button" data-toggle="tooltip" title="Add Refund Amount" onclick="addRefundAmount(<?php echo $customer['customer_block_id'] . ',' . $customer['batch']; ?>);"> <i class="fa fa-plus"></i> </button>
                                                    <?php }
                                                }
   ...
   function deleteRefundTransaction(refund_id, transaction_hist_id){
        if(confirm('Are you sure?')){
            $.ajax({
                url:'../dynamic/deleteRefundTransaction.php',
                type:'post',
                data:{
                    refund_id:refund_id
                },
                success:function(resp){
                    getRefundTransactions(transaction_hist_id);
                }
            });
        }else{
            return false;
