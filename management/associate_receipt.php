...
$transaction_details = mysqli_fetch_assoc(mysqli_query($conn,"select id, customer_id, associate_id, block_id, block_number_id, payment_type, bank_name, cheque_dd_number, amount, cr_dr, paid_date, addedon from kc_associates_transactions where id = '".$transaction_id."' and status = '1' limit 0,1 "));

if(!isset($transaction_details['id']) || $transaction_details['cr_dr'] != 'dr' || !($transaction_details['amount'] > 0)){
	die;
}

$associate_details = mysqli_fetch_assoc(mysqli_query($conn,"select * from kc_associates where status = '1' and id = '".$transaction_details['associate_id']."' limit 0,1 "));
...
$block_number_details = mysqli_fetch_assoc(mysqli_query($conn,"select block_number from kc_block_numbers where id = '".$transaction_details['block_number_id']."' limit 0,1 "));
?>
<!doctype html>
<html>
<head>
    ...
    <table width="92%" border="0" style="margin-left:auto; margin-right:auto;">
      <tbody>
        ...
        <tr>
          ...
          <td colspan="6" align="center" style="font-size:20px; font-weight:bolder; text-transform:uppercase; ">Associate Receipt</td>
        </tr>
        <tr>
        	<td colspan="6">&nbsp;</td>
        </tr>
        <tr>
          <td width="15%" nowrap>Associate Code</td>
          <td width="2%">:</td>
          <td width="58%"><?php echo sprintf("%05d",$associate_details['id']); ?></td>
        ...
        <tr>
          ...
          <td>:</td>
          <td colspan="4" nowrap><?php echo $associate_details['name']; ?></td>
        </tr>
        <tr>
          <td colspan="6">&nbsp;</td>
        </tr>
          ...
          <td colspan="6"><table width="100%" border="1" style="border-collapse:collapse;">
            <tbody>
              ...
              <tr>
                ...
                <td><?php echo $block_number_details['block_number']; ?> - PLOT NO. <?php echo $block_details['name']; ?></td>
