...
$transaction_details = mysqli_fetch_assoc(mysqli_query($conn,"select * from kc_avr_receipt where id = '".$transaction_id."' and status = '1' limit 0,1 "));

if(!isset($transaction_details['id']) || !($transaction_details['paid_amount'] > 0)){
	die;
}
    ...
    <table width="92%" border="0" style="margin-left:auto; margin-right:auto;">
      <tbody>
        ...
        <tr>
          ...
          <td>:</td>
          <td colspan="4" nowrap><?php echo $transaction_details['name_title'].' '.$transaction_details['name']; ?></td>
        </tr>
       <!--  <tr>
          <td>Co-owner(s)</td>
          <td>:</td>
        ...
        <?php if(trim($transaction_details['parent_name']) != ''){ ?>
          <tr>
            <td><?php echo $transaction_details['parent_name_relation']; ?> of </td>
            <td>:</td>
            <td colspan="1"><?php echo $transaction_details['parent_sub_title']; ?> <?php echo $transaction_details['parent_name']; ?></td>
          </tr>
        <?php }else{ ?>
        ...
        <tr>
          ...
          <td>:</td>
          <td><?php echo ucfirst($transaction_details['payment_type']); //($transaction_details['addedon'] > date("Y-m-d",strtotime("2018-10-26")) && $transaction_details['payment_type'] == "Cash")?"Payment":$transaction_details['payment_type']; ?></td>
        </tr>
        <tr>
          ...
          <td colspan="6"><table width="100%" border="1" style="border-collapse:collapse;">
            <tbody>
              <tr>
                ...
                <td align="center"><strong>Project Details</strong></td>
                <td align="center"><strong>Paid Amount (Rs.)</strong></td>
              </tr>
              <tr>
                <td align="center">01</td>
                <td><?php echo $transaction_details['project_block_plotnumber_totalarea']; ?></td>
                <td align="center"><?php echo number_format($transaction_details['paid_amount']); ?></td>
              </tr>
            </tbody>
          </table></td>
        </tr>
        ...
        <tr>
          <td nowrap><strong><?php echo numberToWord($transaction_details['paid_amount']); ?></strong></td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
