...
if(isset($_POST['type']) && $_POST['type'] == 'booked'){
	$query .= " and id IN (select block_number_id from kc_customer_blocks where status = '1')";
}else if(!isset($_POST['type']) || $_POST['type'] != 'all'){
	$query .= " and id NOT IN (select block_number_id from kc_customer_blocks where status = '1')";
}
