...
function outStandingPayments($conn,$associateID =0){
  ...
  $query = "select ct.customer_id,mt.associate, ct.block_id, ct.block_number_id from kc_customer_transactions ct";
  if($associateID > 0){
    $query .= " INNER JOIN kc_customer_blocks cb ON cb.customer_id = ct.customer_id and cb.block_id = ct.block_id and cb.block_number_id = ct.block_number_id JOIN kc_associate_percentage as mt on ct.customer_id = mt.customer_id where mt.associate = '".$associateID."' and cb.status = '1' and cb.status = '1' group by cb.customer_id, cb.block_id, cb.block_number_id";
  }else{
  ...
  $return_array = [];
    ...
    $total= isOutStandingPayment($conn,$customer['customer_id'],$customer['block_id'],$customer['block_number_id']);
		...
		if(isOutStandingPayment($conn,$customer['customer_id'],$customer['block_id'],$customer['block_number_id'])){
			...
			$return_array[$counter]['associate_id'] = $customer['associate'];
...
function blockNumberAssociateDetails($conn,$customerID,$blockID,$blockNumberID){
  $block_associate_details = (mysqli_query($conn,"select associate, associate_percentage from `kc_associate_percentage` where customer_id = '".$customerID."' and block_id = '".$blockID."' and block_number_id = '".$blockNumberID."' and status = '1'  limit 0,7 "));
  $block_assocaite=[];
  while($row=mysqli_fetch_assoc($block_associate_details)){
    $block_assocaite[]=$row;
  }

  return (isset($block_assocaite) && $block_assocaite > 0)?$block_assocaite:array();
}
function makeAssociateCredit($conn,$transactionID){
  ...
  if(isset($transaction_details['customer_id']) && !associateHadCommissionForTransaction($conn,$transactionID)){
    ...
    foreach( $block_associate_details as $key=>$value){
      if(isset($value['associate']) && isset($value['associate_percentage']) && isset($value['associate_percentage']) > 0){
        $associate_commision = round(($transaction_details['amount']*$value['associate_percentage'])/100);
          if(!mysqli_query($conn,"insert into kc_associates_transactions set customer_id = '".$transaction_details['customer_id']."', block_id = '".$transaction_details['block_id']."', block_number_id = '".$transaction_details['block_number_id']."', transaction_id = '$transactionID', associate_id = '".$value['associate']."', amount = '".$associate_commision."', cr_dr = 'cr', paid_date = '".date("Y-m-d")."', status = '1', addedon =NOW(), added_by = '".$_SESSION['login_id']."' ")){
            return false;
...
function associateTotalCreditedHistory($conn,$associateID){
  $total_cr = mysqli_fetch_assoc(mysqli_query($conn,"select amount as total_credited from kc_associates_transactions where associate_id = '".$associateID."' and status = '1' and cr_dr = 'cr' "));
  if(!empty($total_cr['total_credited'])){
    return $total_cr['total_credited'];
  }else{
    return 0.00;
  }
...
function associateCommission($conn,$associateID){
  $total_commission =mysqli_fetch_assoc(mysqli_query($conn,"select associate_percentage from kc_associate_percentage where associate = '".$associateID."' and status = '1' "));
  $data=[];
  if(!empty($total_commission)){
    // while($total_co = mysqli_fetch_assoc($total_commission)){
    //     $data[]=$total_co;
    // }
...
function associateTotalDebitedHistory($conn,$associateID){
  $total_cr = mysqli_fetch_assoc(mysqli_query($conn,"select amount as total_credited from kc_associates_transactions where associate_id = '".$associateID."' and status = '1' and cr_dr = 'dr' "));
...
function getSmsBalance(){
  ...
  $url = "http://sms.quickinfotech.co.in/api/balance.php?authkey=e295c16f00c29cb5f3f892d4b7ab8848&route=B";
